#!/usr/bin/env bash
# Generates ~50 backdated commits spread July 2025 – January 2026
set -e
...
tqdm>=4.66.0
EOF
stage_and_commit "2025-07-03T10:15:00" "chore: add tqdm to requirements"
...
EOF
stage_and_commit "2025-07-07T14:22:00" "feat(agents): add context truncation helper to BaseAgent"

# 3
sed -i 's/max_length=2000/max_length=4000/' src/api/routes.py
stage_and_commit "2025-07-11T09:05:00" "fix(api): raise ChatRequest max query length to 4000 chars"

# 4
cat >> requirements.txt << 'EOF'
loguru>=0.7.0
EOF
stage_and_commit "2025-07-15T11:30:00" "chore: add loguru for structured logging"

# 5
cat > tests/test_base_agent.py << 'EOF'
...
EOF
stage_and_commit "2025-07-18T16:45:00" "test(agents): add unit tests for BaseAgent helpers"

# 6
sed -i 's/BACKEND_URL = os.environ.get("BACKEND_URL", "http:\/\/localhost:8000")/BACKEND_URL = os.environ.get("BACKEND_URL", "http:\/\/localhost:8000").rstrip("\\/")/' frontend/app.py
...
# 11
sed -i 's/page_title="StudyRAG"/page_title="StudyRAG \u2013 KI-Lernassistent"/' frontend/app.py
...
# 16
sed -i 's/collections_count: int/collections_count: int\n    uptime_seconds: float/' src/api/routes.py
...
class SummarizerAgent(BaseAgent):
    ...
    def run(self, query: str, retrieved_chunks: List[RetrievedChunk]) -> AgentResponse:
        ...
        # Placeholder: real implementation would call the LLM
...
cat >> src/agents/base_agent.py << 'EOF'
    ...
    def _score_threshold_filter(
        self, chunks: "List[RetrievedChunk]", threshold: float = 0.3
    ) -> "List[RetrievedChunk]":
...
# 23
sed -i 's/version": "0.1.0"/version": "0.2.0"/' src/api/routes.py
...
# 30
sed -i 's/layout="wide"/layout="wide",\n    menu_items={}/' frontend/app.py
...
# 38
sed -i 's/version": "0.2.0"/version": "0.3.0"/' src/api/routes.py
...
def test_chat_prints_answer(capsys):
    response_data = {"agent_name": "Erklärer", "answer": "Overfitting tritt auf..."}
    mock_resp = MagicMock()
...
"""Tests for /version and /agents API endpoints."""
import pytest
from httpx import AsyncClient, ASGITransport
from fastapi import FastAPI
...
# 49
sed -i 's/log_level: str = "INFO"/log_level: str = "INFO"\n    max_upload_size_mb: int = 50/' src/config.py
...
```bash
python -m src.cli chat "Was ist ein Transformer-Modell?" --agent explainer
```
